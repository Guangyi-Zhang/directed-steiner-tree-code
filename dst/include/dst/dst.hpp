...
namespace dst {
  ...
  public:
    ...
    auto level3_alg_outdated(const double alpha) {
      // dijktra from the root
      ...
      auto tree3 = std::make_shared<PartialTree> (root);
      while (terms_left.size() > 0) {
        ...
        for (auto u: V) {
          ...
          //fmt::println("V_: {}", V_);
          auto tree_u = std::make_shared<PartialTree> (root);
